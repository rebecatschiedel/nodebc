...
const games = [
  ...
  games.forEach(game => {
      for (i = 0; i < propertiesNames.length; i++)
        console.log(`${propertiesNames[i]}: ${game[propertiesNames[i]]}`);      
  });


  //only console log the value of the property
  games.forEach(game => {
    propertiesNames.forEach(property => console.log(game[property]));      
  });
